...
// @Description Accept or reject a judged submission. A rejected submission keeps its
// @Description verdict but stops counting towards the team's best score.
// @Tags Submissions
// @Accept json
// @Produce json
// @Param id path string true "Submission ID"
...
func (h *handler) reviewSubmission(c *gin.Context) {
	...
	// A rejection that no one can explain later is the one an organizer regrets, so
	// the reason is required for it and meaningless for the reverse.
