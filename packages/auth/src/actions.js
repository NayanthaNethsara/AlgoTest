...
export async function authenticateUser(credentials, expectedRole, cookieName = SESSION_COOKIE) {
...
export async function revokeUserSession(cookieName = SESSION_COOKIE) {
    const cookieStore = await cookies();
...
export const fetchSessionUser = cache(async (cookieName = SESSION_COOKIE) => {
    const cookieStore = await cookies();
    const sessionToken = cookieStore.get(cookieName)?.value;
    if (!sessionToken) {
        return null;
