...
/**
 * Execute custom user code against custom stdin on the contest server.
 *
 * Contestants run and test in their own IDE with their own compilers; this path is
 * only the portal's scratchpad, and it is never gated on agent liveness so an
 * agent hiccup cannot stop someone testing their code.
 */
