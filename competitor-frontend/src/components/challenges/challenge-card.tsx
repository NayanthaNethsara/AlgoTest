...
export function ChallengeCard({
    ...
    const cardContent = (
      ...
      >
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between gap-2">
            ...
            {isLocked ? (
              <Badge
                ...
                className="gap-1 text-[10px] uppercase px-2 py-0.5"
              >
          ...
          <div>
            <h3 className={`font-bold text-sm sm:text-base text-foreground leading-snug ${!isLocked ? "group-hover:text-primary transition-colors" : ""}`}>
              {problem.title}
            </h3>
            {problem.statement && (
        ...
        <div className="flex items-center justify-between border-t-2 border-border pt-3 mt-auto">
          ...
          <div
            className={`flex h-8 w-8 items-center justify-center pixel-flat ${
              isLocked
                ? "bg-muted text-muted-foreground"
                : "bg-primary text-primary-foreground transition-transform group-hover:scale-105"
            }`}
          >
            {isLocked ? (
    ...
    >
      <div className="flex flex-col gap-2 min-w-0 flex-1">
        <div className="flex items-center gap-3">
          <span className={`font-bold text-xs sm:text-sm text-foreground truncate ${!isLocked ? "group-hover:text-primary transition-colors" : ""}`}>
            {problem.title}
          </span>
          <Badge
            variant={difficultyVariant}
      ...
      <div className="flex items-center gap-5 shrink-0">
        ...
        <div
          className={`flex h-8.5 w-8.5 items-center justify-center pixel-flat ${
            isLocked
              ? "bg-muted text-muted-foreground"
              : "bg-primary text-primary-foreground transition-transform group-hover:scale-105"
          }`}
  ...
  return (
    <Link href={`/challenges/${problem.slug}`} prefetch={true}>
      {listCardContent}
    </Link>
  );
}
